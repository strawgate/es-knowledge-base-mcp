...
class AskServer:
    ...
    async def questions(self, questions: list[str], answer_style: QuestionAnswerStyle = QuestionAnswerStyle.NORMAL) -> list[AskQuestionResponse]:
        """Ask questions of the knowledge base."""

        question_results = zip(
            questions,
            await self.knowledge_base_server.search_kb_all(queries=self._questions_to_queries(questions, answer_style=answer_style)),
        )

        return [AskQuestionResponse(question=question, results=results) for question, results in question_results]
    ...
    async def questions_for_kb(
        ...
        question_results = zip(
            ...
            await self.knowledge_base_server.search_kb(
                knowledge_base=knowledge_base, queries=self._questions_to_queries(questions, answer_style=answer_style)
            ),
        )
